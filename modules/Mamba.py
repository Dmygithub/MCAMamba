...
class FSSBlock(nn.Module):
    def __init__(
        ...
        self.interaction_awareness = nn.Sequential(
    ...
    def forward(self, x, y):
        ...
        if self.use_adaptive and not self.training:
            with torch.no_grad():
                x_mean = torch.mean(x_1, dim=1, keepdim=True)
                y_mean = torch.mean(y_1, dim=1, keepdim=True)
                modal_info = torch.cat([x_mean, y_mean], dim=1)
                self.interaction_score = self.interaction_awareness(modal_info)
...
class SpecMambaBlock(nn.Module):
    ...
    def forward(self, input):
        ...
        if input_dim == 3: 
            ...
            raise ValueError(f"Unexpected input dimension: {input_dim}, expected 3 or 5")
